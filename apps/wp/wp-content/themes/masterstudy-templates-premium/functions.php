...
function stm_ms_lms_theme_register_styles() {
    ...
    wp_enqueue_style(
        'stm_lms_starter_theme_css_frontend',
        MS_LMS_STARTER_THEME_URI . '/assets/css/style.css',
        array(),
        $version
    );
    wp_enqueue_style(
        'masterstudy-starter-elementor-icons',
        MS_LMS_STARTER_THEME_URI . '/assets/fonts/elementor/icons/style.css',
        null,
        STM_THEME_VERSION,
        'all'
    );
...
if ( !function_exists( 'masterstudy_starter_fs' ) && file_exists( STM_THEME_PATH . '/freemius/start.php' ) ) {
    function masterstudy_starter_fs() {
        ...
        if ( !isset( $masterstudy_starter_fs ) ) {
            ...
            $masterstudy_starter_fs = fs_dynamic_init( array(
                'id'              => '16465',
